...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            elif path=="/api/review-blind-session-start": result=s.jobs.submit("Creating sealed blind listening session",lambda:s.review_blind_session_start(data))
            elif path=="/api/review-blind-session-submit": result=s.jobs.submit("Recording sealed blind session answer",lambda:s.review_blind_session_submit(data))
            elif path=="/api/review-blind-session-history": result=s.review_blind_session_history(data)
