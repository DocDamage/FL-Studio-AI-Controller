...
async function refreshAssets(){assets=await api("assets");const audio=assets.filter(a=>a.kind==="input"||a.kind==="audio");for(const id of ["audio-asset","reference-asset"]){const old=$(id).value;$(id).innerHTML=`<option value="">${id==="audio-asset"?"Choose working bounce":"Choose reference / candidate"}</option>`+audio.map(a=>`<option value="${a.id}">${esc(a.name)} · ${esc(a.kind)}</option>`).join("");if(audio.some(a=>a.id===old))$(id).value=old;}window.dispatchEvent(new Event("flcopilot-assets-updated"));}
